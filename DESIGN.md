...
### Icons
A hand-drawn set in `src/components/Icon.astro`: a 24px grid, no fill, `currentColor` stroke at 1.75 with round caps and joins. The set grows only when a real page need appears. Every icon is decorative, sits beside its own Thai label, and is hidden from assistive tech. Icons drawn here are drawn deliberately, not copied: the hamburger and the drawer close mark match the same grid and stroke, and the LINE speech bubble is drawn by hand rather than pulled from a downloaded brand asset.

### Image Placeholder
Every image slot on the site uses `ImagePlaceholder.astro` with real descriptive Thai `alt` text plus a small visible Thai label telling the owner what belongs there. The placeholder shows a hairline border over a 6% honey tint. Passing a real `src` clears the placeholder treatment and label automatically; optional `srcset`, `sizes`, crop position, and loading priority let the same slot ship efficiently from phone to wide desktop. Default aspect ratio is 16/10.
